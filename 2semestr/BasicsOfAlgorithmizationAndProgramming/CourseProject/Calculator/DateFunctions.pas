...
const
  tdays: array [1 .. 12] of integer = (31, 28, 31, 30, 31, 30, 31, 31, 30, 31,
    30, 31); // Массив количеств дней в месяцах

const
  Сnamemonths: array [1 .. 12] of string[10] = ('Января', 'Февраля', 'Марта',
...
// процедура для перевода дней и месяцев в строку формата (ddmm)
procedure prepare(var a, b, c: integer; k: string);
procedure ConvToDate(a, b, c: integer; var s: string);
// процедура для перевода дней,месяцев,лет в формат даты
...
begin
  ...
  begin
    ...
    result := inttostr(d) + ' ' + string(Сnamemonths[m]) + ' ' + inttostr(y) +
      ' ' + 'Года';
  end;
end;
...
begin
  ...
  // Проход по строке и проверка всех символов
  begin
    inc(i);
  end;
  if (i <= length(x)) or (x = '') then
  // если найден неподходящий символ,то результат-false, иначе результат-true
  begin
    result := false;
  end
  else
...
begin
  ...
  case c of // Конвертация номера года
    0 .. 9:
      s := s + '000';
      // Если год однозначное число,то нужно добавить перед ним "000"
    10 .. 99:
      s := s + '00'; // Если двухзначное,то нужно добавить "00"
    100 .. 999:
      s := s + '0'; // Если однозначное,то нужно добавить "0"
  end;
...
begin
  ...
  begin
    ...
    // Проходимся по годам и если есть високосные добавляем еще по дню
    begin
  ...
  begin
    for i := (m2 + 1) to 12 do
    // Проходимся по месяцам меньшей даты с месяца следующего за текущим до конца года
    ...
    // Добавляем к разнице количество дней оставшихся до конца месяца в текущем месяце второй даты
    if (m2 = 2) and ((leapyearcheck(y2) = true)) then
    // Если это високосный год,то добавляем еще день
      inc(diff);
    for i := 1 to m1 - 1 do
    // Проходимся от начала года до месяца, предшествующего текущему месяцу первой даты
    begin
      if (i = 2) and (leapyearcheck(y1) = true) then
      // Если високосный год и февраль,то добавляю еще днеь
        inc(diff);
  ...
  begin
    diff := 0;
    if m1 = m2 then
    // Если месяца дат совпадают, то разница равна разнице между номерами дней
      diff := abs(d1 - d2)
    else
    begin
      ...
      /// недостающих во второй дате для перехода на следующий месяе
        inc(diff);
      ...
      begin
        ...
        begin
          diff := diff + tdays[i];
          // Добавление к разнице количества дней в этих месяцах
          if (leapyearcheck(y2) = true) and (i = 2) then
          // Если попался високосный год  и 2 месяц,то еще день добавляю
            inc(diff);
        end;
      end;
    end;
...
begin
  ...
  tmp := tdays[m] - d + 1;
  // помещаем количество дней до перехода на следующий месяц
  if (leapyearcheck(y) = true) and (m = 2) then
  // Проверка на високосность года и на февраль
    inc(tmp);
  if x < tmp then
  // если число добавляемых дней меньше, чем нужно для перехода на следующий месяц
  begin
    d := d + x;
  end
  else
  begin
    repeat
      ...
      if (leapyearcheck(y) = true) and (m = 2) then
        inc(tmp);
      x := x - tmp;
      // Отнимаем от добавляемых дней число дней, необходимое для перехода не след месяц
      if m = 12 then
      // Если номер месяца-12,то переход на первый месяц следующего года
      begin
        m := 1;
        y := y + 1;
      end
  ...
  if months < 13 - m then
  // Если количество добавляемых месяцев меньше, чем нужно для перехода на следующий гол
    m := m + months
  else
  begin // Иначе
    repeat
    ...
    until months < 13 - m;
    // Пока количество добавляемых месяцев не меньше,чем нужно для перехода на следующий год
    m := m + months;
  end;
  y := y + years;
  result := '';
...
begin
  ...
  begin
    repeat
      ...
      if m = 1 then
      // Если месяц был первым, то меняем на последний месяц предыдущего года
      begin
        m := 12;
        dec(y);
      end
      ...
      d := tdays[m];
      // Текущему числу дней присваиваем количество дней,равное дням в месяце
      if ((leapyearcheck(y) = true)) and (m = 2) then
      // Проверка на високосный год и месяц февраль
        inc(d);
    until x < d;
    // До тех пор,пока количество отнимаемых дней не станет меньше текушего числа дней месяца                                //Отнимаем от текущего числа оставшиеся дни
  end;
  d := d - x;
  if months >= m then
  // Количество отнимаемых месяцев больше либо равно номеру текущего месяца
  begin
    repeat // Если да,то повторять:
      months := months - m;
      // От отнимаемого числа месяца отнимаем номер текущего месяца
      m := 12; // Переход на 12 месяц предыдущего года
      dec(y); //
    until months < m;
    // До тех пор, пока число отнимаемых месяцев не станет меньше номера текущего месяца
  end;
